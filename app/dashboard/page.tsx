        ...
        <div className="flex items-center gap-3">
          ...
          <div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center font-bold text-slate-400 text-xs overflow-hidden border border-slate-100">
            {avatarUrl ? (
              ...
              <span>{pseudonym ? pseudonym[0].toUpperCase() : '?'}</span>
            )}
          </div>
        </div>
      </header>
              ...
              <div key={story.id} className="border border-slate-100 p-8 rounded-[32px] flex flex-col md:flex-row justify-between items-start md:items-center bg-white hover:shadow-lg transition-shadow">
                ...
                <div className="w-full md:w-auto flex items-center gap-3">
                  ...
                  <button 
                    ...
                    className="p-4 rounded-2xl bg-red-50 text-red-500 hover:bg-red-100 transition-colors border border-red-100"
                  >
                  ...
                  {isVotingActive ? (
                    <div className="bg-orange-50 border border-orange-100 px-6 py-4 rounded-2xl text-center flex-1 md:min-w-[200px]">
                       ...
                       <button disabled className="text-slate-400 text-sm font-bold cursor-not-allowed italic">Ожидайте</button>
                    </div>
                  ) : (
                    <Link 
                      href={`/dashboard/add-chapter?storyId=${story.id}&next=${(lastChapter?.chapter_number || 0) + 1}`}
                      className="inline-block text-center bg-slate-900 text-white px-8 py-4 rounded-2xl font-bold hover:bg-blue-600 transition shadow-lg flex-1 md:min-w-[200px]"
                    >
